...
public class PeriodControl {
	...
	private static void elabora()  {
		...
		try {
			String sql = "select pc.C_PERIODCONTROL_ID, pc.PERIODSTATUS from C_Year y join C_Period p on p.C_YEAR_ID = y.C_YEAR_ID join C_PeriodControl pc on pc.C_PERIOD_ID = p.C_PERIOD_ID where y.ad_client_id = ? and y.fiscalyear = ?";
	...
	/*public static void OpenFromSql(String sql)  {
	...
	private static void Open(ResultSet rs) {
		
		try {
			
			if (rs.getString(2).compareTo("O") == 0)
				return;
			
			MPeriodControl p = new MPeriodControl(Env.getCtx(), rs.getInt(1), null);
			
			p.set_ValueOfColumn("PeriodAction", "O");
			p.save();
